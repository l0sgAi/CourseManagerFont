...
const submit = async () => {
  if (!courseData.value.id) {
    //传入的courseData中无id值，是插入方法
    const { code } = await SaveCourseData(courseData.value)
    if (code === 200) {
      //提交成功状态码
      //关闭弹窗
...
const deleteById = row => {
  ElMessageBox.confirm('此操作将永久删除该记录，确定删除？', '删除确认', {
    ...
    const { code } = await DeleteCourseData(row.id)
    if (code === 200) {
      //成功状态码
      //提示信息
